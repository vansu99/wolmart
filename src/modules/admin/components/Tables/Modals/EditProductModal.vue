<template>
  <modal
    modal-width="700"
    modal-title="Chỉnh sửa chi tiết sản phẩm"
    :modal-active="dialog"
    modal-color="#fff"
    modal-position="end"
    :modal-button="[
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      formData.append('is_gift', is_gift);
      ...
      formData.append('is_free_shipping', is_free_shipping);

      try {
        const response = await productApis.updateProduct(formData, id);
